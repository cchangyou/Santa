...
from model.optimizers import SGD, Momentum, RMSprop, Adam
...
def train_model(tr, va, te, n_x=88, n_h=200, patience=10, max_epochs=200, 
    lrate=0.001, optimizer='Adam', dispFreq=10, validFreq=200, 
    ...
    if optimizer=='SGD':
        f_grad_shared, f_update = SGD(tparams, cost, [x], lr)
    elif optimizer=='SGD-M':
        f_grad_shared, f_update = Momentum(tparams, cost, [x], lr)
    elif optimizer=='RMSprop':
        f_grad_shared, f_update = RMSprop(tparams, cost, [x], lr)
    elif optimizer=='Adam':
        f_grad_shared, f_update = Adam(tparams, cost, [x], lr)
...
if __name__ == '__main__':
    ...
    logger = logging.getLogger('eval_{}_{}'.format(dataset, algorithm))
    logger.setLevel(logging.INFO)
    fh = logging.FileHandler('eval_{}_{}.log'.format(dataset, algorithm))
    ...
    saveto = "{}_{}".format(dataset, algorithm)
    ...
    [tr_negll, va_negll, te_negll] = train_model(tr, va, te, lrate=1.*1e-3, 
        validFreq=validFreq,optimizer=algorithm, saveto = saveto)
